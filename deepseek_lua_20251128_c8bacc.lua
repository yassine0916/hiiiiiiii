...
ESPToggle.Position = UDim2.new(0.25, 0, 0.25, 0)
...
AutoAimToggle.Position = UDim2.new(0.75, 0, 0.25, 0)
...
HitboxToggle.Position = UDim2.new(0.25, 0, 0.75, 0)
...
local BulletAimToggle = Instance.new("TextButton")
BulletAimToggle.Size = UDim2.new(0, 50, 0, 50)
BulletAimToggle.Position = UDim2.new(0.75, 0, 0.75, 0)
BulletAimToggle.AnchorPoint = Vector2.new(0.5, 0.5)
BulletAimToggle.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
BulletAimToggle.BorderSizePixel = 0
BulletAimToggle.Text = "BULLET"
BulletAimToggle.TextColor3 = Color3.fromRGB(255, 255, 255)
BulletAimToggle.Font = Enum.Font.GothamBold
BulletAimToggle.TextSize = 10
BulletAimToggle.Parent = MainFrame
...
for _, button in pairs({ESPToggle, AutoAimToggle, HitboxToggle, BulletAimToggle, CloseButton}) do
...
-- BULLET AIM FUNCTION (Bullets go to enemy torso)
local BulletAimConnection = nil

local function GetTorsoPosition(player)
    if not player.Character then return nil end
    
    -- Try to find torso (works for both R6 and R15)
...
local function RedirectBullets()
    if not getgenv().NxrHub.BulletAim then return end
    if not LocalPlayer.Character then return end
    
    local closestPlayer = GetClosestPlayer()
    if not closestPlayer then return end
    
    local torsoPosition = GetTorsoPosition(closestPlayer)
    if not torsoPosition then return end
    
    -- Find and redirect bullets/weapons
    ...
    if tool then
        ...
        if handle then
            -- Make tool aim at enemy torso
            handle.CFrame = CFrame.lookAt(handle.Position, torsoPosition)
        end
    end
    
    -- Redirect any projectiles
    for _, obj in pairs(workspace:GetChildren()) do
        if obj:IsA("BasePart") and obj.Name:lower():find("bullet") or obj.Name:lower():find("projectile") then
            if obj:GetAttribute("Owner") == LocalPlayer then
                obj.CFrame = CFrame.lookAt(obj.Position, torsoPosition)
            end
        end
    end
end

local function StartBulletAim()
    if BulletAimConnection then
        BulletAimConnection:Disconnect()
    end
    BulletAimConnection = RunService.Heartbeat:Connect(RedirectBullets)
end

local function StopBulletAim()
    if BulletAimConnection then
        BulletAimConnection:Disconnect()
        BulletAimConnection = nil
    end
end
...
ToggleButton.MouseButton1Click:Connect(function()
    ...
    if menuOpen then
        ...
        TweenService:Create(MainFrame, TweenInfo.new(0.3), {Size = UDim2.new(0, 220, 0, 220)}):Play()
...
-- Bullet Aim Toggle Function
BulletAimToggle.MouseButton1Click:Connect(function()
    getgenv().NxrHub.BulletAim = not getgenv().NxrHub.BulletAim
    if getgenv().NxrHub.BulletAim then
        BulletAimToggle.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
        StartBulletAim()
        print("Bullet Aim Enabled - Bullets will go to enemy torso")
    else
        BulletAimToggle.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
        StopBulletAim()
        print("Bullet Aim Disabled")
    end
end)
...
addButtonEffect(BulletAimToggle)
...
print("🎯 Bullet Aim: Ready - Bullets go to enemy torso")