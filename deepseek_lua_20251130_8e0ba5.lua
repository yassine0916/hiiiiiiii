...
local Config = {
    ...
    DistanceESP = true,
    TeamCheck = true,
    MaxDistance = 500 -- تقليل المسافة القصوى لتقليل اللاغ
}
...
-- =============================================
-- وظائف الرؤية عبر الجدران المحسنة للهاتف
-- =============================================
...
local function CreateESP(player)
    local esp = {
        Box = CreateDrawing("Square", {
            Thickness = 3, -- زيادة السماكة للرؤية الأفضل على الهاتف
            Filled = false,
            Color = ESPColor,
            Visible = false
        }),
...
local function CalculateBox(playerCharacter)
    ...
    local boxWidth = boxHeight * 0.7 -- زيادة العرض للرؤية الأفضل
...
-- متغير للتحكم في معدل تحديث الـ ESP لتقليل اللاغ
local lastUpdate = 0
local UPDATE_INTERVAL = 0.1 -- تحديث كل 0.1 ثانية بدلاً من كل إطار
...
local function UpdateESP()
    ...
    -- التحكم في معدل التحديث لتقليل اللاغ
                    ...
                    if distance <= Config.MaxDistance then
                        ...
                        if valid then
                            ...
                            if Config.NameESP then
                                esp.Name.Position = Vector2.new(boxPosition.X + boxSize.X / 2, boxPosition.Y - 25) -- زيادة المسافة للرؤية الأفضل
                            ...
                            if Config.DistanceESP then
                                esp.Distance.Position = Vector2.new(boxPosition.X + boxSize.X / 2, boxPosition.Y + boxSize.Y + 8) -- زيادة المسافة للرؤية الأفضل
...
local function initializeSystem()
    ...
    print("📱 نظام ESP محسن للهاتف - Boxes, Names, Distance")
    print("🎯 نظام الهيت بوكس المتقدم - مع شريط تحكم الحجم")
    print("⚡ أداء محسن - تقليل اللاغ")
